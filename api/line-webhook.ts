...
async function processWebhookEvents(bodyText: string, ctx: ReqCtx) {
  ...
  for (const ev of events) {
    try {
      ...
      // 通帳・小口現金出納帳：取引明細を bank_transactions にまとめて保存
      ...
      // 給与明細／賃金台帳：従業員行を payroll_lines にまとめて保存（人件費=経費）
      if (docType === 'payslip' || docType === 'wage_ledger') {
        const lines: any[] = Array.isArray(doc?.lines)
          ? doc.lines.filter((l: any) => l && typeof l === 'object')
          : [];
        if (lines.length === 0) {
          await saveUnprocessed({ ...saveCtx, docType }, 'no_payroll_lines_found');
          if (ev.replyToken) await replyLineMessage(ctx.accessToken, ev.replyToken, '給与明細から読み取れる項目が見つかりませんでした。');
          continue;
        }
