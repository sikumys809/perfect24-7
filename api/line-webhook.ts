...
document_type が receipt / invoice / tax_payment の場合のスキーマ（含まれる書類を1件ずつ全件）:
{
  "document_type": "receipt | invoice | tax_payment",
  "receipts": [
    {
...
async function saveReceipt(ex: any, ctx: SaveCtx): Promise<string> {
  ...
  // 納付書は【納付】＋税目で表示。それ以外は売上/経費
  const isTax = ctx.docType === 'tax_payment';
  const head = isTax ? `【納付】${ex?.tax_kind ?? '税金・社保'}` : direction === 'sales' ? '【売上】' : '【経費】';
  const who = isTax ? ex?.vendor ?? counterparty ?? '納付先不明' : counterparty ?? '取引先不明';
  return `${head} ${who} / ${issued ?? '日付不明'} / ${yen}${feeStr}${warn}`;
}
