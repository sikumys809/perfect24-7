...
async function saveInventory(
  ...
  if (low > 0) fieldRows.push({ receipt_id: receipt?.id ?? null, field_name: 'validation_notes', field_value: `信頼度が低い行が${low}件`, source: 'validation' });
  await supabase.from('extracted_fields').insert(fieldRows);
...
async function processWebhookEvents(bodyText: string, ctx: ReqCtx) {
  ...
  for (const ev of events) {
    try {
      ...
      // 棚卸表：在庫明細を document_lines にまとめて保存（期末在庫金額）
      if (docType === 'inventory') {
        const lines: any[] = Array.isArray(doc?.lines)
          ? doc.lines.filter((l: any) => l && typeof l === 'object')
          : [];
        if (lines.length === 0) {
          await saveUnprocessed({ ...saveCtx, docType }, 'no_inventory_lines_found');
          if (ev.replyToken) await replyLineMessage(ctx.accessToken, ev.replyToken, '棚卸表から読み取れる品目が見つかりませんでした。');
          continue;
        }
        const asOf = typeof doc?.as_of_date === 'string' ? doc.as_of_date : null;
        const { count, total } = await saveInventory(lines, { ...saveCtx, imageHash: fileHash, docType }, asOf);
        if (ev.replyToken) {
          const tally = await monthlyTallyLine(client.id);
          await replyLineMessage(ctx.accessToken, ev.replyToken, `棚卸表を登録しました（${count}品目・在庫金額¥${total.toLocaleString()}）${tally}`);
        }
        continue;
      }

      // 借入金返済予定表：返済明細を document_lines にまとめて保存
      if (docType === 'loan_schedule') {
        const lines: any[] = Array.isArray(doc?.lines)
          ? doc.lines.filter((l: any) => l && typeof l === 'object')
          : [];
        if (lines.length === 0) {
          await saveUnprocessed({ ...saveCtx, docType }, 'no_loan_lines_found');
          if (ev.replyToken) await replyLineMessage(ctx.accessToken, ev.replyToken, '返済予定表から読み取れる明細が見つかりませんでした。');
          continue;
        }
        const lender = typeof doc?.lender === 'string' ? doc.lender : null;
        const { count } = await saveLoanSchedule(lines, { ...saveCtx, imageHash: fileHash, docType }, lender);
        if (ev.replyToken) {
          const tally = await monthlyTallyLine(client.id);
          const head = lender ? `返済予定表（${lender}）` : '返済予定表';
          await replyLineMessage(ctx.accessToken, ev.replyToken, `${head}を登録しました（${count}回分）${tally}`);
        }
        continue;
      }
