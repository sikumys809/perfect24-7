  ...
  "receipts": [
    {
      ...
      "vendor": "発行元（この書類を発行した店名・会社名）。不明なら null",
      "recipient": "宛名・請求先・宛先（この書類を受け取る側の会社/個人名。例:「○○御中」「○○様」）。無ければ null",
      "direction": "提出者（顧問先）から見た向き。顧問先が発行した側=sales（売上）、顧問先が受け取った/支払う側=expense（経費）。判断できなければ null。下部の【提出者】の指示に従うこと",
      "total_incl_tax": "税込合計金額＝この取引の主たる金額（数値のみ）。不明なら null。重要: 振込手数料などの付随する少額は主金額にしない（下記の fee に入れる）",
...
async function saveReceipt(ex: any, ctx: SaveCtx): Promise<string> {
  ...
  const dir = resolveDirection(ex, ctx.clientName);
  const direction = dir ?? 'expense'; // 不明は経費として仮置き（needs_review で拾う）
  const counterparty =
  ...
  // 向きが判定できなかった場合も要確認に含める
  const notes = [...validation.notes];
  if (dir === null) notes.push('売上/経費の自動判定ができず経費として仮置き');
  const needsReview = validation.needsReview || dir === null;
  fieldRows.push({
    receipt_id: receipt?.id ?? null,
  ...
  const side = direction === 'sales' ? '【売上】' : '【経費】';
  return `${side}${counterparty ?? '取引先不明'} / ${issued ?? '日付不明'} / ${yen}${feeStr}${warn}`;
}
